...
#[ we disable nimsl altogether because it clashes many definitions with nim-glm
proc myVertexShader(trans: mat4, aTexcoord: vec2, aPos: vec2, vTexcoord: var vec2): vec4 =
  vTexcoord = aTexcoord
  result = trans * newVec4(aPos, 0, 1)
...
proc main =
  ...
  var unitrans = glGetUniformLocation(shaderProgram, "trans")
  var trans = mat4f(1).rotate(radians(180'f32), vec3f(0, 0, 1))
  unitrans.glUniformMatrix4fv(1, GL_FALSE, addr trans[0][0])
  ...
  while windowShouldClose(window) == 0:
    ...
    trans = trans.rotate(diff * 180.0.radians, vec3f(0, 0, 1))
    unitrans.glUniformMatrix4fv(1, GL_FALSE, addr trans[0][0])
