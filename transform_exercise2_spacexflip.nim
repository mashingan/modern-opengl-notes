...
proc main =
  ...
  func ftransform(val: float32): float32 =
    (1 - exp(-val)) / (1 + exp(-val))
    #1 / (1 + exp(-val))
  while windowShouldClose(window) == 0:
    ...
    if spacePush and flipInc > 180.0:
      spacePush = false
      flipInc = 0
    elif spacePush:
      let rotspace = 5 * ftransform(flipInc)
      model = model.rotate(rotspace.radians, axis)
      flipInc += 1.0
    ...
    if window.getKey(KEY_ESCAPE) == 1 or window.getKey(KEY_Q) == 1 or
      window.getMouseButton(MOUSE_BUTTON_RIGHT) == 1:
      window.setWindowShouldClose(1)
    elif window.getKey(KEY_SPACE) == 1 or
      window.getMouseButton(MOUSE_BUTTON_LEFT) == 1:
      spacePush = true
